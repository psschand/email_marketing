...
update_database_hostnames() {
    print_header "FIX DATABASE HOSTNAMES"
...
begin
    ...
    begin
        if ActiveRecord::Base.connection.table_exists?(:users)
            ...
            if email_col
                admin_user = begin
                    User.where(email_col => 'admin@postal.example.com').first
                rescue => _
                    nil
                end
                if admin_user
                    ...
                    updated += 1
                    changes << "User: admin@postal.example.com -> ${NEW_ADMIN_EMAIL}"
                end

                normal_user = begin
                    User.where(email_col => 'user@postal.example.com').first
                rescue => _
                    nil
                end
                if normal_user
                    ...
                    updated += 1
                    changes << "User: user@postal.example.com -> ${NEW_USER_EMAIL}"
                end
            else
    ...
    begin
        if ActiveRecord::Base.connection.table_exists?(:domains)
            Domain.where(name: 'postal.example.com').find_each do |d|
                d.update(name: '${BASE_DOMAIN}')
                updated += 1
                changes << "Domain: postal.example.com -> ${BASE_DOMAIN}"
            end
        else
    ...
    begin
        if ActiveRecord::Base.connection.table_exists?(:servers)
            Server.where(name: 'postal.example.com').find_each do |s|
                s.update(name: '${BASE_DOMAIN}')
                updated += 1
                changes << "Server: postal.example.com -> ${BASE_DOMAIN}"
            end
        else
...
show_interactive_menu() {
    while true; do
        ...
        echo "6. Run Full Setup"
                echo "7. Fix Database Hostnames (replace postal.example.com)"
                echo "8. Exit"
        echo
